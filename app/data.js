...
const CASE_CARDS = [
  ...
  {
    id: 'C011',
    title: 'Seedance 多圖投喂誤判',
    outcome: 'success',
    task: 'storyboard',
    models: ['seedance'],
    problem: '曾以為 Seedance 只支持單圖 first_frame——多圖測試失敗。實際敗因是 prompt 太複雜且與故事卡內容衝突，不是圖數量限制。',
    method: '同時投喂兩張參考圖（人物卡 + 故事版卡），prompt 極簡化為一句「使用這個角色，按故事版分鏡生成視頻」；鏡頭/動作/節奏信息全部畫進故事卡。',
    reusableRule: '信息進圖不進 prompt：參考圖承載結構信息時，prompt 越短越穩；先懷疑 prompt 衝突再懷疑接口限制。',
    appMapping: ['生成器', '框架指南', '清洗器'],
    tags: ['seedance', 'reference', 'storyboard', 'prompt-conflict'],
    nextAction: '把「一句話投喂」加入 Seedance 導出模板。'
  },
  {
    id: 'C012',
    title: '整張故事版直接投喂',
    outcome: 'success',
    task: 'storyboard',
    models: ['seedance', 'gpt-image-2'],
    problem: '逐鏡裁切成單張圖再逐個投喂，流程重且鏡頭銜接斷裂。',
    method: '用 gpt-image-2 生成一張 16:9 故事版圖（多個分鏡幀橫排 + 編號 + 鏡頭參數 + 動作標注），整圖直接投喂 Seedance，模型自己識別序列。',
    reusableRule: '區分「製作板」（給人看的完整展示）與「故事版圖」（給模型看的乾淨幀序列）；後者只留幀 + 標注。',
    appMapping: ['生成器', '框架指南'],
    tags: ['storyboard', 'seedance', 'filmstrip', 'multi-frame'],
    nextAction: '故事版圖版式做成可套用模板。'
  },
  {
    id: 'C013',
    title: '逐集視頻續接保一致性',
    ...
    task: 'mv-shot',
    models: ['seedance'],
    problem: '多集連續視頻用靜態人物卡逐集生成，角色仍會跨集漂移。',
    method: '第 1 集用素材參考（@圖片）生成；第 2 集起用「將@視頻1延長15s」——以前一集視頻為參考續接，一致性由模型自身視頻上下文保證。',
    reusableRule: '長敘事視頻的一致性錨點優先用「前一段視頻」而非靜態圖；素材編號系統（C01/S01/P01）讓引用可管理。',
    appMapping: ['生成器', '記憶卡'],
    tags: ['seedance', 'continuity', 'series', 'video-reference'],
    nextAction: '把素材編號約定寫進分鏡框架提示。'
  },
  {
    id: 'C014',
    title: 'Reference ≠ 像素鎖底',
    outcome: 'failure',
    task: 'product-photo',
    models: ['nova'],
    problem: '商品為白編織繩 + 半透明琥珀樹脂環。只傳 reference 圖時，上游 LLM 看圖誤判樹脂為金屬並寫進 prompt，「gold-bronze metallic」強文字壓過參考圖，成品漂成實心金屬環。',
    method: '三道鎖：① reference 傳原圖 ② PRODUCT TRUTH 段寫死真實材質/色/形（半透明樹脂≠金屬）③ 保真鎖前置為最高優先級（圖文衝突時圖贏）。鎖對材質後賣點文案也自動對齊。',
    reusableRule: '參考機制是「參照重畫」不是 img2img 像素鎖；prompt 裡錯誤的強文字會壓過參考圖——保真 = 識別準 + 逐項硬約束 + 錨定前置，三者缺一即漂。',
    appMapping: ['生成器', '清洗器', 'QA 檢查'],
    tags: ['nova', 'reference-lock', 'material', 'fidelity', 'drift'],
    nextAction: '把 PRODUCT TRUTH 段式加入商品圖模板。'
  },
  {
    id: 'C015',
    title: '服裝漂移與模型選型',
    ...
    task: 'character-card',
    models: ['nova', 'gpt-image-2'],
    problem: '服裝搭配場景下生成型模型按文字「重畫」服裝，顏色/款式/材質與參考圖不一致；文字描述越詳細漂得越狠。',
    method: '還原參考圖細節的場景換參考主導的模型；場景描述裡禁寫服裝細節（顏色/款式/材質一概不寫），只寫場景/動作/氛圍/光線，字數 80-150。',
    reusableRule: '需要還原（人物/服裝/產品）→ 參考主導模型 + 極簡文字；純生成 → 生成型模型。文字與圖片會競爭主導權。',
    appMapping: ['模型路由', '生成器'],
    tags: ['wardrobe', 'drift', 'model-routing', 'reference'],
    nextAction: '模型路由矩陣補「還原 vs 生成」選型行。'
  },
  {
    id: 'C016',
    title: '結構化短 Prompt 勝過堆砌',
    outcome: 'success',
    task: 'product-photo',
    models: ['gpt-image-2'],
    problem: '8000+ 字超長 spec 或 500+ 詞堆砌 prompt 反而讓出圖質量下降。',
    method: '100-200 詞結構化短標籤分段（場景→主體→細節→約束+用途）；參考圖按 index+描述引用並說明交互；防漂移句「change only X, keep everything else the same」每輪重複；全圖文字 <50 詞，生僻詞逐字母拼。',
    reusableRule: '清晰度 > 長度：意圖與約束清晰的 100-200 詞結構化 prompt 實測優於長文堆砌約 80%。',
    appMapping: ['清洗器', '生成器', '框架指南'],
    tags: ['gpt-image-2', 'prompt-length', 'structure', 'anti-drift'],
    nextAction: '清洗器加「超長 prompt 壓縮建議」規則。'
  },
  {
    id: 'C017',
    title: '生圖端點能力差異',
    ...
    problem: '同一模型不同端點能力不同：文生圖端點支持 4K，圖生圖/編輯端點上限 1536×1024——按文檔上限拍腦袋會踩空。',
    method: '上線前對每個端點實測分辨率上限與參數支持（quality=high 等）；把端點差異寫進模型路由說明。',
    reusableRule: '「模型支持 X」≠「你用的端點支持 X」；分辨率/參考圖/質量參數逐端點驗證。',
    appMapping: ['模型路由', '測試矩陣'],
    tags: ['endpoint', 'resolution', 'api', 'verification'],
    nextAction: '測試矩陣補端點×分辨率場景。'
  },
  {
    id: 'C018',
    title: '品牌 VI 素材不用 AI 位圖',
    outcome: 'success',
    task: 'aplus-page',
    models: ['gpt-image-2'],
    problem: '品牌 favicon/og 圖若用 AI 生成位圖：插畫感與極簡體系衝突、改字重跑全靠抽卡、矢量體系無法融入。',
    method: '改用腳本繪製（SVG/PIL 超採樣）：像素精確、可重跑、改文案配色只改參數；AI 生圖留給內容創作而非品牌資產。',
    reusableRule: '可參數化的品牌資產用代碼生成；「告別抽卡」的場景就不要引入抽卡。',
    appMapping: ['QA 檢查', '案例庫'],
    tags: ['brand', 'vi', 'svg', 'deterministic', 'tooling'],
    nextAction: '無。'
  }
];
